(() => {
  ...
  async function api(path, options = {}) {
    const target = path.startsWith("/ui/api/")
      ? path
  ...
  function renderServices(statusByService) {
    ...
    if (role === "operator") {
      // streaming-overlay.service handles overlays and GPU encoding; the plain
      // service remains available when neither is requested.
    }
  }
  ...
  function freshness(lastSeenAt) {
    if (!lastSeenAt) return { label: "Sin comunicación", className: "inactive" };
    const age = Math.max(0, (Date.now() - new Date(lastSeenAt).getTime()) / 1000);
    if (!Number.isFinite(age) || age > 90) return { label: "Sin comunicación", className: "inactive" };
    if (age > 30) return { label: "Atrasada", className: "warning" };
    return { label: "Comunicada", className: "active" };
  }

  function mediaSummary(media, config) {
    const video = media?.video || {};
    const audio = media?.audio || {};
    const videoName = video.name || video.device || config.VIDEO_DEVICE || "Auto-detectar";
    const videoFormat = video.format || config.VIDEO_INPUT_FORMAT || "formato auto";
    const videoSize = video.width && video.height ? `${video.width}×${video.height}` : `${config.STREAM_WIDTH || "?"}×${config.STREAM_HEIGHT || "?"}`;
    const videoFps = video.fps || config.VIDEO_INPUT_FPS || config.STREAM_FPS || "?";
    const audioName = audio.name || audio.device || config.AUDIO_DEVICE || "Silencio AAC";
    const audioReason = audio.priority_reason || audio.reason || "selección automática";
    return {
      camera: `${videoName} · ${videoFormat} · ${videoSize}@${videoFps}`,
      audio: `${audioName} · ${audioReason}`,
      video: `${config.STREAM_WIDTH || "?"}×${config.STREAM_HEIGHT || "?"} · ${config.STREAM_FPS || "?"} FPS · ${Math.round(Number(config.STREAM_BITRATE || 0) / 1000) || "?"} kbps`,
      encoder: config.GPU_ENCODER === "true" ? "GPU · h264_v4l2m2m" : "CPU · libx264",
    };
  }
  ...
  function renderRemoteOverview(state, media, config) {
    const health = state.last_health || {};
    const status = freshness(state.last_seen_at);
    const stateEl = $("remote-connection-state");
    const badge = $("remote-last-seen");
    stateEl.textContent = status.label;
    badge.textContent = state.last_seen_at ? new Date(state.last_seen_at).toLocaleString() : "Sin reporte";
    badge.className = `badge ${status.className}`;
    $("remote-wifi").textContent = health.wifi_ssid || "-";
    ...
    $("remote-camera").textContent = summary.camera;
    $("remote-audio").textContent = summary.audio;
    $("remote-video").textContent = summary.video;
    $("remote-encoder").textContent = summary.encoder;
    const diagnostics = {
      last_seen_at: state.last_seen_at || null,
      services: health.services || {},
      recent_stream_errors: health.recent_stream_errors || "",
      media: media || {},
    };
    $("diagnostics-view").textContent = JSON.stringify(diagnostics, null, 2);
  }

  async function copyRemoteText(text, button) {
    if (!text || text === "-") return;
    try {
  ...
  $("remote-ssh-copy")?.addEventListener("click", () =>
    copyRemoteText($("remote-ssh").textContent.trim(), $("remote-ssh-copy"))
  );
  ...
  async function refreshStatus() {
    ...
    try {
      const results = await Promise.allSettled([
        http.request(`/ui/api/raspi/${DEVICE_ID}/state`),
        api("/api/status"),
        api("/api/devices"),
        api("/api/config"),
      ]);
      ...
      const status = results[1].status === "fulfilled" ? results[1].value : {};
      const devices = results[2].status === "fulfilled" ? results[2].value : {};
      const config = results[3].status === "fulfilled" ? results[3].value : {};
      renderRemoteOverview(state, devices.media || state.last_health?.media || {}, config);
      renderServices(status);
      updatePreviewStatus(status["preview"]);
      lockOverlayToggle(anyServiceActive(status));
      ...
      const message = err.code === "timeout"
        ? "La Raspi tardó demasiado en responder."
        : err.code === "raspi-unavailable"
          ? "La Raspi está desconectada o su túnel no responde."
          : err.code === "backend-unavailable"
            ? "El backend remoto no está disponible."
            : "No se pudo actualizar el estado; reintentando...";
      setActionStatus(message, "error");
    } finally {
      remoteRefreshInFlight = false;
      $("dashboard-view").setAttribute("aria-busy", "false");
    }
  }
  ...
  // The remote portal does not proxy SSE. Poll the meter only while Audio is
  // open, keeping the 10-second operational poll as the only background loop.
  let micPollTimer = null;

  function openMicStream() {
    if (role !== "operator" || micPollTimer) return;
    const msg = $("mic-status");
    if (msg) { msg.className = "device-status"; msg.textContent = "Midiendo…"; }
    const poll = async () => {
      if (!isAudioAccordionOpen() || document.visibilityState !== "visible") return closeMicStream();
      await refreshMic();
      micPollTimer = setTimeout(poll, 10_000);
    };
    poll();
  }

  function closeMicStream() {
    if (micPollTimer) { clearTimeout(micPollTimer); micPollTimer = null; }
  }
  ...
  async function loadConfig() {
    ...
    try {
      ...
      // The Raspi is the source of truth. Never put existing transmission
      // secrets back into visible inputs or summaries.
  ...
  $("logo-file-input").addEventListener("change", async () => {
    ...
    try {
      const data = await api("/api/logo", { method: "POST", body: formData, formData: true, dedupe: false });
  ...
  function validateConfig(intended) {
    ...
    // Empty secret inputs deliberately mean "preserve the configured value".
    // The API remains responsible for rejecting a truly unconfigured target.
  }
  ...
  if (hasRemoteSession()) {
    ...
    showDashboard();
    buildPreviewShell();
    Promise.all([refreshStatus(), loadConfig(), loadPreviewConfig()]).then(() => {
      syncMicStream();
      startRemotePolling();
    });
  }
