(() => {
  ...
  window.createStreamingApiClient = function createStreamingApiClient({
    basePath = "", getHeaders = () => ({}), credentials = "same-origin", timeoutMs = 15_000,
  } = {}) {
      ...
      const promise = (async () => {
        ...
        try {
          const response = await fetch(`${basePath}${path}`, {
            ...
            body: formData ? body : body == null ? undefined : JSON.stringify(body), signal: controller.signal,
          });
          ...
          if (!response.ok) {
            ...
            throw new StreamingApiError(data.error || data.detail || `Error ${response.status}`, { status: response.status, path, code });
          }
          return data;
        } catch (error) {
